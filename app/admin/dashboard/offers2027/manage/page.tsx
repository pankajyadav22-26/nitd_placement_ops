...
export default function ManageOffers() {
  ...
  const fetchOffers = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/manageOffer2027/get/${rollNo}`);
  ...
  const handleDelete = async (offerId: string) => {
    ...
    const res = await fetch(
      `/api/admin/manageOffer2027/delete/${rollNo}/${offerId}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSave = async (offerId: string, updated: any) => {
    setStatus("🔄 Saving...");
    const res = await fetch(
      `/api/admin/manageOffer2027/edit/${rollNo}/${offerId}`,
      {
        method: "PATCH",
